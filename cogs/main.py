...
class SaldoPainelView(discord.ui.View):
    ...
    async def btn_bonus(self, inter, btn):
        await inter.response.defer(ephemeral=True)
        from utils.database import bonus_info, get_bonus_config, saldo_total_usuario
        uid = str(inter.user.id)
        ...
        if total == 0:
            em.description = (
                ...
                f"{DOT} A cada **{_br} salas** compradas você ganha **+{_bpc} sala(s) grátis**!\n\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
            )
        else:
            em.add_field(name=f"{STATS}  Suas Compras", value=(
                f"> {CART} Total comprado: **{total} salas**\n"
                f"> {PRESENTE} Bônus gerado: **{bonus_total} salas** (+{_bpc} a cada {_br})\n"
                f"> {ON} Já resgatado: **{resgatado} salas**\n"
                f"> {'🔥' if bonus_disp > 0 else DOT} **Disponível p/ resgatar: {bonus_disp} sala(s)**"
            ), inline=False)
...
class MainCog(commands.Cog):
    ...
    async def prefix_aa(self, ctx, horario: str = None, centavos: str = None):
        ...
        from cogs.botconfig import (
            ...
            get_promo_ativa, set_promo_ativa, clear_promo_ativa,
            get_msg_auto, _build_promo_v2_payload, _post_promo_v2,
        )
        from utils.pix import get_preco_por_sala
        from zoneinfo import ZoneInfo
        from datetime import datetime as _dt

        # +aa off → desliga promoção
        if (horario or "").lower() in ("off", "desligar", "fim", "0"):
            promo = await asyncio.to_thread(get_promo_ativa)
            if not promo:
                return await ctx.send(embed=_err("Nenhuma promoção ativa no momento."))
            preco_original = promo.get("preco_original_centavos", 9)
            cfg = await asyncio.to_thread(carregar_cfg)
            ...
            em = _ok("Mega Promoção encerrada!", f"{DOLLAR} Preço voltou para **{preco_original} centavos** (R$ {preco_original/100:.2f}/sala).")
            await ctx.send(embed=em)
            return

        # Valida argumentos
        if not horario or not centavos:
            em = _err("Uso correto", f"`+aa HH:MM CENTAVOS` — ex: `+aa 00:00 3`\n`+aa off` — encerra a promo")
        ...
        try:
            ...
            return await ctx.send(embed=_err("Horário inválido.", "Use o formato `HH:MM`, ex: `23:30`"))
        ...
        # Salva preço original e ativa promoção
        preco_atual_cts = round(get_preco_por_sala() * 100)
        novo_preco      = round(cts / 100, 4)

        cfg = await asyncio.to_thread(carregar_cfg)
        cfg["preco_por_sala"] = novo_preco
        cfg["mega_promo"] = {
        ...
        em = _emb(f"{PRESENTE}  Mega Promoção ATIVADA!", config.COR_SUCESSO)
        em.description = (
            f"{DOLLAR} Novo preço: **{cts} centavos** (R$ {novo_preco:.2f}/sala)\n"
            f"{CLOCK} Encerra às: **{h:02d}:{m:02d} BRT**\n"
            f"{REFRESH} Mensagem promo será enviada nos canais configurados.\n\n"
            f"-# Use `+aa off` para encerrar antes do horário."
        )
        await ctx.send(embed=em)
    ...
    async def on_interaction(self, inter: discord.Interaction):
        ...
        if cid.startswith("cw:"):
            ...
            if action == "bonus":
                ...
                em.set_thumbnail(url=inter.user.display_avatar.url)
                if b["total_comprado"] == 0:
                    _br2, _bpc2 = get_bonus_config()
                    em.description = f"{OFF} Você ainda não comprou salas.\n\n{DOT} A cada **{_br2} salas** compradas = **+{_bpc2} sala(s) grátis**!"
                else:
                    em.add_field(name=f"{STATS} Suas Compras", value=(
                        f"> Total: **{b['total_comprado']}** salas\n"
                        f"> Bônus disponível: **{b['bonus_disponivel']}** salas\n"
                        f"> Já resgatado: **{b['bonus_resgatado']}** salas"
                    ), inline=False)
                _br3, _bpc3 = get_bonus_config()
                falta = b["falta_proximo"]; prog = int(((_br3 - falta) / _br3) * 10)
                em.add_field(name=f"{GIFT} Próximo Bônus", value=f"`{'█'*prog}{'░'*(10-prog)}` {_br3-falta}/{_br3}\nFaltam **{falta} salas**!", inline=False)
                return await inter.followup.send(embed=em, view=BonusPainelView(b["bonus_disponivel"], saldo, b["total_comprado"]), ephemeral=True)
